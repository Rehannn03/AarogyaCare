...
export default function HomeScreen() {
  ...
  // Use our custom hook to get medicines
  const { medicines, isLoading } = useMedicines()

  // Filter for active medications only
  const activeMedications = medicines.filter((med) => med.status === "active")
      ...
      renderBackground={() => (
        <View style={styles.headerBackground}>
          <ThemedView
            ...
            darkColor="rgba(96, 165, 250, 0.05)"
          />
        </View>
      )}
      renderForeground={() => (
        <SafeAreaView edges={["top"]} style={styles.header}>
          <View>
            <ThemedText style={styles.greeting}>Hello, Sarah</ThemedText>
        ...
        <ThemedView style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <TouchableOpacity style={styles.seeAllButton} onPress={() => router.push("/medications")}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.medicationsScroll}>
            {isLoading ? (
              ...
              activeMedications.map((med, index) => <MedicationCard key={index} medication={med} />)
            ) : (
              <ThemedText style={styles.emptyText}>No medications found</ThemedText>
            )}
          </ScrollView>
        </ThemedView>
      </Animated.View>
      ...
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        <ThemedView style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity style={styles.seeAllButton} onPress={() => router.push("/features")}>
              <ThemedText style={styles.seeAllText} lightColor={primaryColor} darkColor={primaryColor}>
                See all
              </ThemedText>
              <ArrowRight size={16} color={primaryColor} />
